...
This repository contains a self-contained, lightweight Python pipeline and Jupyter Notebook for generating a **dense 3D point cloud** from a single 2D image.
...
- **`3d_reconstruction/`**: Core folder containing the implementation and outputs:
...
The point cloud is exported to a Stanford PLY file (`3d_reconstruction/output_pointcloud.ply`). 
...
1. **MeshLab** (Recommended, Free, Open-Source):
   ...
   - Go to `File -> Import Mesh...` and select `3d_reconstruction/output_pointcloud.ply`.
...
For the full comparison of modern 3D reconstruction methods and the detailed equations of the camera projection model, read the [3d_reconstruction/notes.md](file:///d:/VS_Code_Projects/PMW-Day1/3d_reconstruction/notes.md) file.
